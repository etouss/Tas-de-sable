...
\begin{eqnarray*}
  ...
  2(|T|-|T'|)(m^2+n^2) & \leq & \sum_{i\leq m,j\leq n} (i^2 + (m-i)^2 + j^2 + (n-j)^2)*T(i,j) \\
...
\end{array}\right. $
\begin{eqnarray*}
  2(|T|-|T'|)(m^2+n^2) & \leq & E(i+1,j) * ((i+1)^2 + (m-(i+1))^2 + j^2 + (n-j)^2) \\
...
Or $\sum E'(x,y) = |T|-|T'|$
Donc on a juste a montré que : $2(m^2+n^2) < (x^2 + (m-x)^2 + y^2 + (n-y)^2)$ est absurde (Inégalité strict grace au 8).
\begin{eqnarray*}
  2(m^2+n^2) & < & (x^2 + (m-x)^2 + y^2 + (n-y)^2)\\
  &&2*(m + n) < -2mx -2ny
\end{eqnarray*}
or $x \in [-1;m]$ et $y \in [-1;n]$
donc $max_x(-2mx) = 2m$ et $max_y(-2ny) = 2n$
En conclusion $2*(m + n) < -2mx -2ny$ est absurde.
